...
export default function ReturJualPage() {
  ...
  const [editingReturJual, setEditingReturJual] = useState<ReturJual | null>(null)
  const [viewingReturJual, setViewingReturJual] = useState<ReturJual | null>(null)
  ...
  const handleApprove = async (returJual: ReturJual) => {
    try {
      const response = await fetch(`/api/transaksi/retur-jual/${returJual.id}/approve`, {
        method: 'POST',
      })
  ...
  const handleComplete = async (returJual: ReturJual) => {
    try {
      const response = await fetch(`/api/transaksi/retur-jual/${returJual.id}/complete`, {
        method: 'POST',
      })
  ...
  const handleDelete = async (returJual: ReturJual) => {
    ...
    if (!confirm(`Apakah Anda yakin ingin menghapus transaksi "${returJual.noRetur}"?`)) {
      return
    }
  ...
  const addItem = () => {
    setItems([...items, { barangId: '', qty: 1, harga: 0, alasan: '', kondisi: 'bisa_dijual_lagi' }])
  }
  ...
  const getKondisiBadge = (kondisi: string) => {
    switch (kondisi) {
      case 'bisa_dijual_lagi':
        return <Badge variant="default" className="bg-green-100 text-green-800"><RefreshCw className="w-3 h-3 mr-1" />Bisa Dijual Lagi</Badge>
      case 'rusak_total':
        return <Badge variant="destructive"><AlertCircle className="w-3 h-3 mr-1" />Rusak Total</Badge>
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{statistics.completedCount}</div>
            <p className="text-xs text-muted-foreground">
              Transaksi selesai
            </p>
          </CardContent>
                  ...
                  <TableBody>
                    {returJuals.map((returJual) => (
                      <TableRow key={returJual.id}>
                        <TableCell className="font-medium">{returJual.noRetur}</TableCell>
                        <TableCell>{formatDate(returJual.tanggal)}</TableCell>
                        <TableCell>
                          <div>
                            <div className="font-medium">{returJual.customer.nama}</div>
                            <div className="text-sm text-gray-500">{returJual.customer.kode}</div>
                          </div>
                        </TableCell>
                        <TableCell>{returJual.totalQty}</TableCell>
                        <TableCell>{formatCurrency(Number(returJual.totalNilai))}</TableCell>
                        <TableCell>{getStatusBadge(returJual.status)}</TableCell>
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => handleView(returJual)}>
                                <Eye className="mr-2 h-4 w-4" />
                                Detail
                              </DropdownMenuItem>
                              {returJual.status === 'draft' && (
                                <>
                                  <DropdownMenuItem onClick={() => handleEdit(returJual)}>
                              ...
                              {(returJual.status === 'approved' || returJual.status === 'completed') && (
                                <DropdownMenuItem onClick={() => handlePrint(returJual)}>
              ...
              {pagination.totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-gray-500">
                    Menampilkan {returJuals.length} dari {pagination.total} data
                  </div>
                  <div className="flex space-x-2">
                    <Button
                      variant="outline"
        ...
        <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid gap-4 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  <Label htmlFor="suratJalanId">Ref. Surat Jalan (Opsional)</Label>
                  <Select
                    value={watchedValues.suratJalanId || 'none'}
                    onValueChange={(value) =>
                      setValue('suratJalanId', value === 'none' ? undefined : value)
                    }
                    disabled={submitting}
                  >
                    <SelectTrigger>
                  ...
                  {errors.suratJalanId && (
                    <p className="text-sm text-red-600">{errors.suratJalanId.message}</p>
                  )}
                </div>
                <div className="grid gap-2">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  {items.map((item, index) => (
                    <div key={index} className="grid grid-cols-13 gap-2 items-end">
                      ...
                      <div className="col-span-2">
                        ...
                        <Select
                          value={item.kondisi}
                          onValueChange={(value: 'bisa_dijual_lagi' | 'rusak_total') => updateItem(index, 'kondisi', value)}
                          disabled={submitting}
                        >
                          <SelectTrigger>
                            <SelectValue />
                          ...
                          <SelectContent>
                            <SelectItem value="bisa_dijual_lagi">Bisa Dijual Lagi</SelectItem>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={submitting}>
                {submitting ? 'Menyimpan...' : editingReturJual ? 'Perbarui' : 'Simpan'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={viewDialogOpen} onOpenChange={setViewDialogOpen}>
        <DialogContent className="sm:max-w-[1000px] max-h-[90vh] overflow-y-auto">
            ...
            <Tabs defaultValue="info" className="w-full">
              ...
              <TabsContent value="info" className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>Tanggal</Label>
                    <p className="font-medium">{formatDate(viewingReturJual.tanggal)}</p>
                  </div>
                  <div>
                    <Label>Customer</Label>
                    <p className="font-medium">{viewingReturJual.customer.kode} - {viewingReturJual.customer.nama}</p>
                  </div>
                  <div>
                    <Label>Status</Label>
                    <div className="mt-1">
                  ...
                  <div>
                    ...
                    <p className="font-medium">{viewingReturJual.totalQty} unit</p>
                  </div>
                  <div>
                    <Label>Total Nilai</Label>
                    <p className="font-medium">{formatCurrency(Number(viewingReturJual.totalNilai))}</p>
                  </div>
                  {viewingReturJual.suratJalan && (
                    <div>
                      <Label>Ref. Surat Jalan</Label>
                      <p className="font-medium">{viewingReturJual.suratJalan.noSJ}</p>
                    </div>
                  )}
                  <div className="col-span-2">
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                  <div>
                    ...
                    <p className="font-medium">{formatDate(viewingReturJual.createdAt)}</p>
                  </div>
                  <div>
                    <Label>Terakhir Diubah</Label>
                    <p className="font-medium">{formatDate(viewingReturJual.updatedAt)}</p>
                  </div>
                </div>
              </TabsContent>
          ...
          <DialogFooter>
            {viewingReturJual && (viewingReturJual.status === 'approved' || viewingReturJual.status === 'completed') && (
              <Button
                variant="outline"
