...
export default function SuratJalanPage() {
  ...
  const [editingSuratJalan, setEditingSuratJalan] = useState<SuratJalan | null>(null)
  const [viewingSuratJalan, setViewingSuratJalan] = useState<SuratJalan | null>(null)
  const [stockCheckResult, setStockCheckResult] = useState<any>(null)
  const [items, setItems] = useState<FormData[]>([
    { barangId: '', qty: 1, hargaJual: 0 }
  ])
  const [deliveryOption, setDeliveryOption] = useState<'partial' | 'complete'>('complete')
  ...
  const fetchMasterData = async () => {
    try {
      const [customersRes, gudangsRes, barangsRes, suppliersRes] = await Promise.all([
      ...
      const [customersData, gudangsData, barangsData, suppliersData] = await Promise.all([
        customersRes.json(),
        gudangsRes.json(),
        barangsRes.json(),
        suppliersRes.json(),
  ...
  const onSubmit = async (data: SuratJalanFormData) => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.dropshipSummary && result.dropshipSummary.dropshipItems > 0) {
          toast.info(`${result.dropshipSummary.dropshipItems} item dropship terdeteksi`)
        }
      } else {
  ...
  const handleCheckStock = async (suratJalan: SuratJalan) => {
    try {
      ...
      const response = await fetch(`/api/transaksi/surat-jalan/${suratJalan.id}/check-stock`, {
        method: 'POST',
      })
  ...
  const handlePost = async (suratJalan: SuratJalan) => {
    try {
      setSubmitting(true)
      const response = await fetch(`/api/transaksi/surat-jalan/${suratJalan.id}/post`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleView = async (suratJalan: SuratJalan) => {
    try {
      const response = await fetch(`/api/transaksi/surat-jalan/${suratJalan.id}`)
      const result = await response.json()
  ...
  const handleDelete = async (suratJalan: SuratJalan) => {
    ...
    if (!confirm(`Apakah Anda yakin ingin menghapus transaksi "${suratJalan.noSJ}"?`)) {
      return
    }
  ...
  const updateItem = (index: number, field: keyof FormData, value: any) => {
    ...
    if (field === 'barangId') {
      ...
      newItems[index] = {
        ...newItems[index],
        [field]: field === 'qty' || field === 'hargaJual' ? Number(value) : value,
      }
    }
    setItems(newItems)
  }
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'in_transit':
        return <Badge variant="default"><Truck className="w-3 h-3 mr-1" />Dalam Pengiriman</Badge>
      case 'delivered':
        return <Badge variant="default"><CheckCircle className="w-3 h-3 mr-1" />Terkirim</Badge>
      case 'cancelled':
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-8">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-600">{statistics.inTransitCount}</div>
            <p className="text-xs text-muted-foreground">
              Sedang dikirim
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">{statistics.deliveredCount}</div>
            <p className="text-xs text-muted-foreground">
              Selesai dikirim
            </p>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-purple-600">{statistics.dropshipCount}</div>
            <p className="text-xs text-muted-foreground">
              Transaksi dropship
            </p>
          </CardContent>
                  ...
                  <TableBody>
                    {suratJalans.map((suratJalan) => (
                      <TableRow key={suratJalan.id}>
                        <TableCell className="font-medium">{suratJalan.noSJ}</TableCell>
                        <TableCell>{formatDate(suratJalan.tanggal)}</TableCell>
                        <TableCell>
                          <div>
                            <div className="font-medium">{suratJalan.customer.nama}</div>
                            <div className="text-sm text-gray-500">{suratJalan.customer.kode}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div>
                            <div className="font-medium">{suratJalan.gudang.nama}</div>
                            <div className="text-sm text-gray-500">{suratJalan.gudang.kode}</div>
                          </div>
                        </TableCell>
                        <TableCell>{suratJalan.totalQty}</TableCell>
                        <TableCell>{formatCurrency(Number(suratJalan.totalNilai))}</TableCell>
                        <TableCell>{getStatusBadge(suratJalan.status)}</TableCell>
                        <TableCell>
                          {suratJalan.detail.some(d => d.isDropship) ? (
                            <Badge variant="outline">
                              <ShoppingCart className="w-3 h-3 mr-1" />
                              {suratJalan.detail.filter(d => d.isDropship).length} Item
                            </Badge>
                          ) : (
                        ...
                        <TableCell className="text-right">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => handleView(suratJalan)}>
                                <Eye className="mr-2 h-4 w-4" />
                                Detail
                              </DropdownMenuItem>
                              {suratJalan.status === 'draft' && (
                                <>
                                  <DropdownMenuItem onClick={() => handleEdit(suratJalan)}>
                                    <Edit className="mr-2 h-4 w-4" />
                                    Edit
                                  </DropdownMenuItem>
                                  <DropdownMenuItem onClick={() => handleCheckStock(suratJalan)}>
              ...
              {pagination.totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <div className="text-sm text-gray-500">
                    Menampilkan {suratJalans.length} dari {pagination.total} data
                  </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="grid gap-6 py-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="grid gap-2">
                  ...
                  />
                  {errors.alamatKirim && (
                    <p className="text-sm text-red-600">{errors.alamatKirim.message}</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="grid gap-2">
                  ...
                  />
                  {errors.namaSupir && (
                    <p className="text-sm text-red-600">{errors.namaSupir.message}</p>
                  )}
                </div>
                <div className="grid gap-2">
                  ...
                  {errors.nopolKendaraan && (
                    <p className="text-sm text-red-600">{errors.nopolKendaraan.message}</p>
                  )}
                </div>
              </div>
              ...
              <div className="grid gap-2">
                ...
                <Select value={deliveryOption} onValueChange={(value: 'partial' | 'complete') => setDeliveryOption(value)}>
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <div key={index} className="grid grid-cols-12 gap-2 items-end border rounded-lg p-3">
                      ...
                      <div className="col-span-2">
                        ...
                        <Input
                          ...
                          onChange={(e) => updateItem(index, 'hargaJual', e.target.value)}
                          disabled={submitting}
                        />
                      </div>
                      <div className="col-span-2">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={submitting}>
                {submitting ? 'Menyimpan...' : editingSuratJalan ? 'Perbarui' : 'Simpan'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
            ...
            <Tabs defaultValue="info" className="w-full">
              ...
              <TabsContent value="info" className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label>Tanggal</Label>
                    <p className="font-medium">{formatDate(viewingSuratJalan.tanggal)}</p>
                  </div>
                  <div>
                    <Label>Customer</Label>
                    <p className="font-medium">{viewingSuratJalan.customer.kode} - {viewingSuratJalan.customer.nama}</p>
                  </div>
                  <div>
                    <Label>Gudang</Label>
                    <p className="font-medium">{viewingSuratJalan.gudang.kode} - {viewingSuratJalan.gudang.nama}</p>
                  </div>
                  <div>
                    <Label>Total Qty</Label>
                    <p className="font-medium">{viewingSuratJalan.totalQty} unit</p>
                  </div>
                  <div>
                    <Label>Total Nilai</Label>
                    <p className="font-medium">{formatCurrency(Number(viewingSuratJalan.totalNilai))}</p>
                  </div>
                  <div>
                    <Label>Status</Label>
                    <div className="mt-1">
                  ...
                  <div>
                    ...
                    <p className="font-medium text-sm">{viewingSuratJalan.alamatKirim}</p>
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <p className="font-medium">{viewingSuratJalan.nopolKendaraan}</p>
                  </div>
                  {viewingSuratJalan.keterangan && (
                    <div className="col-span-2">
                      <Label>Keterangan</Label>
                      <p className="font-medium">{viewingSuratJalan.keterangan}</p>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                  <div>
                    ...
                    <p className="font-medium">{formatDate(viewingSuratJalan.createdAt)}</p>
                  </div>
                  <div>
                    <Label>Terakhir Diubah</Label>
                    <p className="font-medium">{formatDate(viewingSuratJalan.updatedAt)}</p>
                  </div>
                </div>
              </TabsContent>
                    ...
                    <TableBody>
                      {viewingSuratJalan.detail.map((item, index) => (
                        <TableRow key={item.id}>
                          ...
                          <TableCell>{formatCurrency(Number(item.hargaJual))}</TableCell>
                          ...
                          <TableCell>
                            {getDropshipBadge(item.isDropship, item.statusDropship)}
                          </TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                ...
                {viewingSuratJalan.dropshipSummary && (
                  <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                    <Card>
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold">{viewingSuratJalan.dropshipSummary.totalItems}</div>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2">
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-green-600">{viewingSuratJalan.dropshipSummary.normalItems}</div>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2">
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-purple-600">{viewingSuratJalan.dropshipSummary.dropshipItems}</div>
                      </CardContent>
                    </Card>
                    <Card>
                      <CardHeader className="pb-2">
                      ...
                      <CardContent>
                        <div className="text-2xl font-bold text-blue-600">{viewingSuratJalan.dropshipSummary.readyItems}</div>
                      </CardContent>
                    </Card>
                  </div>
                )}
                  ...
                  {viewingSuratJalan.detail
                    ...
                    .map((item) => (
                      <Alert key={item.id}>
                        ...
                        <AlertTitle>Dropship Item: {item.barang.nama}</AlertTitle>
                        <AlertDescription>
                          <div className="space-y-1">
                            <p>Supplier: {item.dropshipSupplier?.nama || 'Tidak ada supplier'}</p>
                            <p>Status: {item.statusDropship || 'Pending'}</p>
                            <p>Qty: {item.qty} {item.barang.satuan}</p>
                          </div>
                        </AlertDescription>
                      </Alert>
                    ))}

                  {viewingSuratJalan.detail.filter(item => item.isDropship).length === 0 && (
      ...
      <Dialog open={stockCheckDialogOpen} onOpenChange={setStockCheckDialogOpen}>
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                <Card>
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">{stockCheckResult.summary.totalItems}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardHeader className="pb-2">
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-green-600">{stockCheckResult.summary.sufficientStockItems}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardHeader className="pb-2">
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold text-orange-600">{stockCheckResult.summary.needsDropshipItems}</div>
                  </CardContent>
                </Card>
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm">Tidak Dapat Dipenuhi</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="text-2xl font-bold text-red-600">{stockCheckResult.summary.cannotFulfillItems}</div>
                  </CardContent>
                </Card>
              </div>
              ...
              <Alert>
                ...
                <AlertTitle>
                  {stockCheckResult.canPost ? 'Bisa Diposting' : 'Tidak Bisa Diposting'}
                </AlertTitle>
                <AlertDescription>
                      ...
                      <TableRow key={index}>
                        <TableCell>
                          <div>
                            ...
                            <div className="text-sm text-gray-500">{item.barang.kode}</div>
                          </div>
                        </TableCell>
                        <TableCell>{item.requestedQty} {item.barang.satuan}</TableCell>
                        <TableCell>
                          <span className={`font-medium ${
                        ...
                        <TableCell>
                          <Badge variant={item.stockStatus === 'sufficient' ? 'default' : 'destructive'}>
                            {item.stockStatus === 'sufficient' ? 'Cukup' : 'Tidak Cukup'}
                          </Badge>
                        </TableCell>
                        <TableCell>
              ...
              {stockCheckResult.warnings.lowStockItems.length > 0 && (
                <Alert>
                  ...
                  <AlertDescription>
                    <div className="space-y-1">
                      {stockCheckResult.warnings.lowStockItems.map((item: any, index: number) => (
                        <p key={index}>
                          {item.barangName}: {item.currentStock} unit (minimum: {item.minStock} unit)
                        </p>
                      ))}
                    </div>
                  </AlertDescription>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setStockCheckDialogOpen(false)}>
              Tutup
            </Button>
